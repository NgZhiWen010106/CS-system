...
import { registerUser, logoutUser } from "../../../redux/actionCreators/authActionCreators"; // Import logoutUser action
...
const Register = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!name || !email || !password) return toast.dark("Please fill in all fields!");

    if (password !== confirmPassword) return toast.dark("Passwords do not match!");

    if (password.length < 8) {
      return toast.dark("Password must be of length 8 or more");
    }
    if (!/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,16}$/.test(password)) {
      return toast.dark("Password must have at least a number and a special character!");
    }
