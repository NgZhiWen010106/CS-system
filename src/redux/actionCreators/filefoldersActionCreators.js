...
export const addFolderUser = (userId, name, formattedDueDate, path, parent) => (dispatch) => {
  database.docs
    .add(docModel(userId, name, formattedDueDate, path, parent))
    .then(async (doc) => {
...
export const addFileUser =
  ({ createdAt, uid, parent, data, yourName, name, url, path }) =>
  (dispatch) => {
    database.files
      .add(fileModel(createdAt, uid, parent, data, yourName, name, url, path))
      .then(async (doc) => {
        const data = await doc.get();
